...
export default Vue.extend({
  data() {
    return {
      goBackUrl: "/vlans",
      captionTranslationKey: "vlans.createHeader",
  ...
  methods: {
    create() {
      ...
      HttpClient.createVlan(item)
          .then((creationResult) => {
            if (creationResult.status === 200 || creationResult.status === 201) {

              this.$router.push(Paths.getVlanDetails(creationResult.data.id));
            }
          })
          .catch((errors) => {
