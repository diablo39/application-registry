<template>
  <v-container>
    <v-row>
      <v-col>
        <v-card>
          <v-section-toolbar :caption="caption"></v-section-toolbar>
          <v-ajax-list-data-source :httpPath="httpPath">
            <template slot-scope="{ ds }">
              ...
              >
                ...
                <template v-slot:item.name="{ item }"><router-link :to="getDetailsUrl(item)">{{ item.name }}</router-link></template>
...
export default Vue.extend({
  ...
  data() {
    return {
      caption: "loadBalancers.header",
      httpPath: HttpClient.getLoadBalancersPath,
      headers: [
        {
  ...
  methods: {
    getDetailsUrl(item): string {
      return Paths.getLoadBalancerDetails(item.name);
    }
  },
});
</script>
