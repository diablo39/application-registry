...
namespace ApplicationRegistry.UnitTests.TestInfrastructure
{
    public class TestDatabaseContext : ApplicationRegistryDatabaseContext
    {
        ...
        private static void SeedSampleData(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<ApplicationVersionSwaggerSpecificationEntity>().HasData(
                new ApplicationVersionSwaggerSpecificationEntity { Id = specificationId, CreateDate = DateTime.UtcNow, IdApplicationVersion = varsionids[0], ContentType = "application/yaml", Code = Guid.NewGuid().ToString(), SpecificationTextHash = Resources.SwaggerDemo.CalculateSHA256() });
